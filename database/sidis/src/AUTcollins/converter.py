...
for f in F:
  ...
  if   hadron=='pip': hadron='pi+'
  elif hadron=='pim': hadron='pi-'
  elif hadron=='pi0': hadron='pi0'
  elif hadron=='Kp': hadron='k+'
  elif hadron=='Km': hadron='k-'
  elif hadron=='K0': hadron='k0'
  elif hadron=='hp': hadron='h+'
  elif hadron=='hm': hadron='h-'
  elif hadron=='h0': hadron='h0'
  else: print 'ERR';sys.exit()

  L=open(f).readlines()
  L=[l.strip() for l in L]
  
  for l in L:
    if '<' and '>' not in l: continue
    if 'stat' in l:  H=l
    if 'sigma' in l: H=l
  ...
  for i in range(len(L)): L[i].extend([target,hadron,col,proj])
  ...
  print '|%d|[link][?]|SIDIS|%s|%s|AUTcollins|%s|%s|'%(idx,target,hadron,col,proj)
