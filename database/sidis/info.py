...
def get_info_file():
  ...
  D={}
  for i in range(len(H)): D[H[i]]=T[i]
  D=pd.DataFrame(D)
  save(D,'.info')
...
if __name__=="__main__":
  ...
  if args.option==0:
    ...
    try:
      ...
      raise ValueError('.info file not found. Use option 0 to generate .info file')
    tab=tab.query('obs=="AUTcollins"')
    compass=tab.query('col=="compass"').sort_values(by=['tar','had','dep'])
    hermes=tab.query('col=="HERMES"').sort_values(by=['tar','had','dep'])
    if args.option==1:
      print compass
      print hermes
    elif args.option==2:
