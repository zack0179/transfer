...
for i in range(len(files)):
    df = pd.read_table(files[i],sep='\s+',names=['xF','value','stat_err_u','sys_err_u','pT'])
    ...
    df['target'] = tar_col
    df['hadron'] = had_col
    if 'pi+' in had_col: df['col'] = b_col
    elif 'pi-' in had_col: df['col'] = b_col
    elif 'pi0' in had_col: df['col'] = s_col
    df['obs'] = obs_col
