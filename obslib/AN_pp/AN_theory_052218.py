...
class ANTHEORY():
    ...
    def get_H1p(self, z, Q2, hadron):
        if (z, Q2) not in self.H1p:
            self.H1p[(z, Q2)] = COLLINS().get_C(z, Q2, hadron) - \
                z * DCOLLINSDZ().get_C(z, Q2, hadron)
        return self.H1p[(z, Q2)]

    def get_H(self, z, Q2, hadron):  # -2*z*H_1^{\perp(1)}(z)+\tilde{H}(z)
        if (z, Q2) not in self.H:
            self.H[(z, Q2)] = -2. * z * COLLINS().get_C(z, Q2,
                                                    hadron) + HTILDE().get_C(z, Q2, hadron)
        return self.H[(z, Q2)]
