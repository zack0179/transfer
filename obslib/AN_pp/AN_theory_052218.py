...
class ANTHEORY():
    ...
    def get_f(self,x,Q2,target): #Collinear unpolarized PDF
        if (x,Q2) not in self.f:
          self.f[(x,Q2)]=PDF().get_C(x,Q2,target)
        return self.f[(x,Q2)]

    def get_ft(self,x,Q2,target): #Collinear unpolarized PDF
        if (x,Q2) not in self.ft:
          self.ft[(x,Q2)]=PDF().get_C(x,Q2,target)
        return self.ft[(x,Q2)]

    def get_d(self,z,Q2,hadron): #Collinear unpolarized FF
        if (z,Q2) not in self.d:
          self.d[(z,Q2)]=FF().get_C(z,Q2,hadron)
        return self.d[(z,Q2)]

    def get_h(self,x,Q2,target): #Collinear transversity
        if (x,Q2) not in self.h:
          self.h[(x,Q2)]=TRANSVERSITY().get_C(x,Q2,target)
        return self.h[(x,Q2)]

    def get_H1p(self,z,Q2,hadron): #(H_1^{\perp(1)}(z) - z*dH_1^{\perp(1)}(z)/dz)
        if (z,Q2) not in self.H1p:
          self.H1p[(z,Q2)]=COLLINS().get_C(z,Q2,hadron) - z*DCOLLINSDZ().get_C(z,Q2,hadron)
        return self.H1p[(z,Q2)]

    def get_H(self,z,Q2,hadron): #-2*z*H_1^{\perp(1)}(z)+\tilde{H}(z)
        if (z,Q2) not in self.H:
          self.H[(z,Q2)]= -2.*z*COLLINS().get_C(z,Q2,hadron) + HTILDE().get_C(z,Q2,hadron)
        return self.H[(z,Q2)]
    ...
    def get_Hupol(self):
        ...
        self.Hupol[10]=32.*self.c['r27']*(self.m['tu']+self.m['ut'])*(1. - 9.*self.m['ts']*self.m['us']*self.c['r4'] )
        self.Hupol[11]=4.*self.c['r9']*(-self.m['su']-self.m['us'])* (1. - 9.*self.m['st']*self.m['ut']*self.c['r4'] )
        self.Hupol[12]=4.*self.c['r9']*(-self.m['st']-self.m['ts'])*(1. - 9.*self.m['su']*self.m['tu']*self.c['r4'] )
        self.Hupol[13]=(self.m['tu']+self.m['ut'])*self.c['r6'] - 3.*(self.m['ts2']+self.m['us2'])*self.c['r8']
        self.Hupol[14]=4.5*(3. - self.m['ts']*self.m['us'] - self.m['st']*self.m['ut'] - self.m['su']*self.m['tu'] )
    ...
    def get_HTffa(self,s,t,u):
        ...
        self.HTffa[1]=-self.c['r9']*self.m['ot'] + self.c['r8']*s*(u-s)*self.m['ot3'] - self.m['st2']*self.m['ou']

        self.HTffa[2]=self.c['r27']*s*(t-u)*self.m['ot2']*self.m['ou'] + self.c['r9']*s*(u-2.*t)*self.m['ot3'] + s*self.m['ot2']

        self.HTffa[3]=self.c['r27']*s*self.m['ot2'] + self.c['r9']*s*(t-s)*self.m['ot3'] - self.c['r3']*self.m['ot']

        self.HTffa[4]=self.c['r27']*s*self.m['ot']*self.m['ou'] - self.c['r3']*self.m['ot']
    ...
    def get_HTffb(self,s,t,u):
        ...
        self.HTffb[1]=self.c['r8']*s*(u-s)*self.m['ot3'] + 0.5*self.c['r9']*(s-u)*self.m['ot']*self.m['ou'] + 0.5*(s-u)*(self.m['t2']-2.*t*u-2.*self.m['u2'])*self.m['ot3']*self.m['ou']

        self.HTffb[2]=self.c['r27']*0.5*s*(t-3.*u)*self.m['ot2']*self.m['ou'] - s*u*self.m['ot3'] + self.c['r9']*s*(2.*u-t)*self.m['ot3'] - self.c['r3']*0.5*self.m['s2']*self.m['ot2']*self.m['ou']

        self.HTffb[3]=self.c['r27']*0.5*(3.*s-t)*self.m['ot2'] + self.m['s2']*self.m['ot3'] + self.c['r9']*s*(t-2.*s)*self.m['ot3'] + self.c['r3']*0.5*u*self.m['ot2']

        self.HTffb[4]=10.*self.c['r27']*0.5*(s-u)*self.m['ot']*self.m['ou']

        self.HTffb[5]=self.c['r9']*s*(2.*u-t)*self.m['ot3'] - s*u*self.m['ot3']

        self.HTffb[6]=self.c['r9']*s*(t-2.*s)*self.m['ot3'] + self.m['s2']*self.m['ot3']
