...
def get_HTffb(s,t,u):
    ...
    HTffb[1]=r8*s*(u-s)*ot3 + 0.5*r9*(s-u)*ot*ou + 0.5*(s-u)*(t2-2.*t*u-2.*u2)*ot3*ou

    HTffb[2]=r27*0.5*s*(t-3.*u)*ot2*ou - s*u*ot3 + r9*s*(2.*u-t)*ot3 - r3*0.5*s2*ot2*ou
...
#   @profile
@jit(float32(float32,float32,float32,float32,float32))
def get_dsigST(x,z,xF,pT,rs): #Calculation of the fragmentation term in the transversely polarized cross section
...
def get_sig(xF,pT,rs,mode='gauss',nx=100,nz=100):
    if mode=='gauss':
      ...
      sig = dblquad(lambda x,z: get_dsig(x,z,xF,pT,rs),zmin,1.,xmin,lambda x: 1.)[0]
    return sig
...
def get_sigST(xF,pT,rs,mode='gauss',nx=100,nz=100):
    if mode=='gauss':
      ...
      sig = dblquad(lambda x,z: get_dsigST(x,z,xF,pT,rs),zmin,1.,xmin,lambda x: 1.)[0]
    return sig
