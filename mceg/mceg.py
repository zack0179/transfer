...
class MCEG:
    ...
    def _gen_event2(self):
        ...
        # make the boost
        Q2 = -self.dot4(q, q)
        B = 1 / Q2**0.5 * np.array([[-q_breit[3], q_breit[0]],
                                [q_breit[0], -q_breit[3]]])
    ...
    def _gen_event(self, R):
        ...
        M = self.conf['aux'].M
        S = self.conf['S']
        if 'pi' in self.conf['hadron']:
            Mh = self.conf['aux'].Mpi
        elif 'k' in self.conf['hadron']:
            Mh = self.conf['aux'].Mk
        else:
        ...
        Spar = self.dot4(S_breit, q_breit) / self.dot4(P_breit,
                                                     q_breit) * M / np.sqrt(1 + gamma**2)
    ...
    def test2(self):
        ...
        for i in range(100):
            ...
            xsec = self.sidis.get_xsec(
                x, z, y, Q2, pT, phi_h, phi_S, Sperp, Spar, le, target, hadron)
            print xsec
