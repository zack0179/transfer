...
conf['params']['transversity']['widths0 valence'] = {'value':<<    3.70971246233364471134e-01>>,'fixed':False,'min':1e-5,'max':2}
conf['params']['transversity']['widths0 sea']     = {'value':<<    4.04126169244575006445e-01>>,'fixed':False,'min':1e-5,'max':2}
conf['params']['transversity']['u N']             = {'value':<<    4.81025963839224157681e+00>> ,'fixed':False,'min':-10,'max':10}
conf['params']['transversity']['u a']             = {'value':<<    5.03096545621322110975e-01>> ,'fixed':False,'min':-1,'max':10}
conf['params']['transversity']['u b']             = {'value':<<    4.90901316844778268234e+00>> ,'fixed':False,'min':1e-5,'max':10}
conf['params']['transversity']['d N']             = {'value':<<   -3.81842696257901224044e+01>> ,'fixed':False,'min':-20,'max':20}
conf['params']['transversity']['d a']             = {'value':<<    1.00219413307881555220e+00>> ,'fixed':False,'min':-1,'max':5}
conf['params']['transversity']['d b']             = {'value':<<    4.66939102717225118511e+00>> ,'fixed':False,'min':1e-5,'max':20}
...
conf['params']['collins']={}
conf['params']['collins']['widths0 pi+ fav']     = {'value':<<    5.77328073218599524541e-02>>,'fixed':False,'min':1e-5,'max':1}
conf['params']['collins']['widths0 pi+ unfav']   = {'value':<<    2.15893243367229648033e-01>>,'fixed':False,'min':1e-5,'max':2}
conf['params']['collins']['pi+ u N 1']  = {'value':<<    1.09578425844584144215e+04>> ,'fixed':False,'min':-10,'max':10}
conf['params']['collins']['pi+ u a 1']  = {'value':<<    9.47773521647447658722e+00>> ,'fixed':False,'min':-1,'max':5}
conf['params']['collins']['pi+ u b 1']  = {'value':<<    8.35392440402659630649e+00>> ,'fixed':False,'min':1e-5,'max':10}
...
conf['params']['collins']['pi+ u N 2']  = {'value':<<    1.00542319996034731910e-02>> ,'fixed':False,'min':-10,'max':10}
conf['params']['collins']['pi+ u a 2']  = {'value':<<   -3.49331588919097635326e+00>> ,'fixed':False,'min':-1,'max':5}
conf['params']['collins']['pi+ u b 2']  = {'value':<<   -3.36004396299020680061e-01>> ,'fixed':False,'min':1e-5,'max':10}
...
conf['params']['collins']['pi+ d N 1']  = {'value':<<   -1.08296228586030792940e+04>> ,'fixed':False,'min':-10,'max':10}
conf['params']['collins']['pi+ d a 1']  = {'value':<<    3.28043657659185150521e+00>> ,'fixed':False,'min':-1,'max':5}
conf['params']['collins']['pi+ d b 1']  = {'value':<<    1.51152834249653675869e+01>> ,'fixed':False,'min':1e-5,'max':10}
