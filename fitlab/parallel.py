  ...
  def run(self):
    ...
    while True:
      ...
      while pending_work and ready_workers:
        ...
        serial += 1
        try:
            backend.send_multipart([worker,'',client,'',repr(serial),repr(offset),repr(self.partition),request],copy=True)
...
class ZmqWorker(object):
  ...
  def toil(self):
    ...
    while True:
      ...
      results.extend([row.func(row.entry)[2] for row in self.bigtable[offset::stride]])
  ...
  def run(self):
    ...
    while len(self.workers):
        for w in list(self.workers):
            if not w.is_alive():
                logging.info('%s terminated with code %d',w.name,w.exitcode)
  ...
  def run_subprocess(self):
      def run_runner():
        ...
        console = logging.StreamHandler()
        console.setLevel(logging.WARN)
        formatter = logging.Formatter('%(asctime)s - slave - %(levelname)s - %(message)s')
        console.setFormatter(formatter)
        logging.root.addHandler(console)
        # os.dup2(log.fileno(),1)
        # os.dup2(log.fileno(),2)
