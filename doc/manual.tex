...
$x,y,\psi,z,\phi_h,P_h^T$. In order to generate the final state 
momenta, one needs to generate samples of  $x,y,\psi,z,\phi_h,P_h^T$
...
$\phi_h$ and $P_h^T$ in \eref{xsec} are defined in the Breit frame. To
...
$\vec{u}=(\vec{q}/|\vec{q}|)\times(-\hat{e}_z)$ 
by an angle $\alpha=\arccos(-(\vec{q}/|\vec{q}|)\cdot \hat{e}_z)$ with 
...
$$
where $a=\cos(\alpha/2)$ and $\vec{w}=\sin(\alpha/2)\vec{u}$. The
...
\begin{enumerate}
\item Generate $E'\in[0,E]$, $\cos(\theta)\in[-1,1]$ and 
...
\end{align}
\item Rotate and boost all the 4-vectors $l,P,l',P_h$ as explained above
